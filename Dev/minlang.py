...
class Interpreter:
    ...
    def exec_stmt(self, node, env):
        ...
        if kind == 'LET':
            ...
            # If __exports__ doesn't exist (file run directly, not via import),
            # silently create it — no error, so standalone linting works fine.
            _, name = node
            try:
                export_list = env.get('__exports__')
                if not isinstance(export_list, list):
                    export_list = []
                    self.global_env.set('__exports__', export_list)
            except NameError:
                export_list = []
                self.global_env.set('__exports__', export_list)
